<template>
	<div class="sc-upload" :class="{ 'sc-upload-round': round }" :style="style">
		...
		<el-dialog title="剪裁1" draggable v-model="cropperDialogVisible" :width="580" @closed="cropperClosed"
			destroy-on-close>
...
export default {
	...
	methods: {
		...
		// 上传功能
		request(param) {
			if (this.folderInfo.file_path_ == undefined) {
				this.$notify.warning({
					title: '警告！',
					message: '请选择上传的文件'
				})
			...
			// 路径信息
			data.append('file_path_', this.folderInfo.file_path_);
